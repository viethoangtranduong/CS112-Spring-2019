...
for (i in seq(0,ceiling(2*max(foo$logcost)),0.1)) {
  X1_treat <- c(1, me.wartype, i, me.wardur, me.factnum, 
                me.factnum2, me.trnsfcap, 1, me.treaty, 
                me.develop, me.exp, me.decade)
  X1_control <- c(1, me.wartype, i, me.wardur, me.factnum, 
                  me.factnum2, me.trnsfcap, 0, me.treaty, 
                  me.develop, me.exp, me.decade)
  
  X2_treat <- c(1, me.wartype, i, me.wardur, me.factnum, 
                me.factnum2, me.trnsfcap, 1, me.treaty, 
                me.develop, me.exp, me.decade, i*1)
  X2_control <- c(1, me.wartype, i, me.wardur, me.factnum, 
                  me.factnum2, me.trnsfcap, 0, me.treaty, 
                  me.develop, me.exp, me.decade, i*0)
  
  model1 <- c(model1, 
              logistic(sum(coef(glm1)*X1_treat)) - logistic(sum(coef(glm1)*X1_control)))
  model2 <- c(model2,
              logistic(sum(coef(glm2)*X2_treat)) - logistic(sum(coef(glm2)*X2_control)))
  
}

plot(x = c(0:315), y = c(0:315), type = "n", 
     xlim = c(0,ceiling(2*max(foo$logcost))), 
     ylim = c(0, 0.8), 
     main = " Causal Effect of Multidimensional UN Peacekeeping Operations", 
     ylab = "Marginal effects of UN peacekeeping operations",
     xlab = 'The log of deaths and displacement')

lines(seq(0, ceiling(2*max(foo$logcost)),0.1), model1, col="black",lty=2, lwd = 2)
text(22, 0.2, "Original Model", col = "black")
lines(seq(0,ceiling(2*max(foo$logcost)),0.1), model2, col="red", lwd = 2)
text(17, 0.7, "Modified Model with \n interaction term", col = "red")
